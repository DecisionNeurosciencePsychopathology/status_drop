...
# just regret
summary(m8r <- lmer(SES ~ edu*time + age*time + INCOMEcst*time + regret*time + SubstanceCurrent*time + (1|ID), data = m))
car::Anova(m8r)

# just neuroticism
summary(m8n <- lmer(SES ~ edu*time + age*time + INCOMEcst*time + neurotic*time + SubstanceCurrent*time + (1|ID), data = m))
car::Anova(m8n)
...
summary(am1 <- lm(mac$SES ~ mac$AGEATT1))
